  ...
  template: `
    ...
    @if (!loading() && detail(); as d) {
      ...
      <!-- Concierge post-signoff CTA — surfaces the offer step when the user
           ...
           v1.5.36 (closes user bug: already offered a price, why is it
           still showing create offer): three variants now:
             1. No offer yet                       → Create buy offer
             2. Offer active (drafted/sent/...)    → Offer already issued, View
             3. Offer terminated (declined/expired)→ Previous offer X, create another
           A latestOffer field landed on InspectionSummaryDto in v1.5.36;
           withdrawn offers are filtered server-side (see SUMMARY_INCLUDE in
           inspections.repo.ts), so a null here means no active offer. -->
      @if (d.kind === 'concierge' && d.status === 'signed_off') {
        ...
        @if (!offer || offerEnded) {
          <!-- Variant 1 + 3: no offer OR previous offer terminated → invite create. -->
          ...
          <!-- Variant 2: an active (non-terminated) offer already exists.
               Don't tempt the admin into duplicating it — link to the existing
               offer's detail page instead. -->
          <div class="bg-emerald-50 rounded-xl border border-emerald-200 p-5 mb-5 flex items-start gap-4 flex-wrap">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-slate-700 mt-0.5">KWD {{ formatFils(offer.amountFils) }} sent to {{ d.customer?.fullName || 'the customer' }}. Open the offer to track customer response, send a counter, or withdraw.</p>
            </div>
...
export class InspectionEditComponent implements OnInit, OnDestroy {
  ...
  /**
   * v1.5.36 — Format InspectionSummaryDto.latestOffer.amountFils (BigInt-as-string
   * over the wire) for KWD display. 1 KWD = 1000 fils → 3-decimal place
   * convention. Returns "—" on parse failure so a corrupt value can't crash
   * the banner.
   */
