...
import { NotificationPrefsGridComponent } from './notification-prefs-grid.component';
...
@Component({
  ...
  imports: [CommonModule, TranslateModule, NotificationPrefsGridComponent],
  template: `
    ...
    <main>
      ...
      @else if (state().kind === 'ready' || state().kind === 'saving') {
        <app-notification-prefs-grid
          ...
          (toggle)="onToggle($event.cat, $event.ch)"
          (save)="onSave()"
        />
